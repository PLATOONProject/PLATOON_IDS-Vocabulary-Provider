    ...
    fs.exists(filePath, function(exists) {
      if (exists) {
        ...
        if (acceptHeader === 'text/turtle' ||
          ...
          if (searchConceptURI) {
          console.log("ENTRA BUSCAR CONCEPTO");
            var queryObject = 'CONSTRUCT{<' + encodeURIComponent(searchConceptURI) + '> ?p ?o .}WHERE {<' + encodeURIComponent(searchConceptURI) + '> ?p ?o .}';
            var endpoint = "http:\/\/fuseki_service_container:"+process.argv.slice(2)[1] || 3030+"/"+res.locals.dataset.name+"/sparql?query="
            console.log("ENTRA BUSCAR CONCEPTO="+endpoint);
            request({
