...
function readTextFile(file, callback) {
 console.log("----------readTextFile------------------");
    ...
    console.log("----------readTextFile------------------"+rawFile.readyState);
    console.log("----------readTextFile------------------"+rawFile.status);
        if (rawFile.readyState === 4 && rawFile.status == "200") {
        console.log("response="+rawFile.responseText);
...
d3sparql.query = function(endpoint, sparql, callback) {
    ...
    console.log(sessionStorage.getItem("dataset_name"));
    console.log("DENTRO DE D3SPARQL DESPUES DE GETITEM");
    var filePath = '/home/adelaida/Proyectos/vocolProyect/vocol/jsonDataFiles/userConfigurations.json';
	...
	/*readTextFile(filePath, function(text){
	    ...
	    console.log("READ THE CONFIGURTAIO FILE");
	    console.log(data);
	});*/
    // read contents of the file with the filePathgetTree
    /*var contents = fs.readFileSync(filePath);
    console.log("d3sparql"+contents);
  ...
  var url = document.URL.split(currentPathString)[0] +
    'fuseki/'+sessionStorage.getItem("dataset_name")+'/sparql' + "?query=" + encodeURIComponent(sparql) +
    '&output=json'
  if (d3sparql.debug) {
...
d3sparql.forcegraph = function(json, config) {
  ...
  function showGraph(nodes, edges, containerName) {
    ...
    $("#result").height('600px').width('800px');
